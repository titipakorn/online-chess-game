...
class SpatialGrid {
  constructor(cellSize) {
    this.cells = new Map(); // Key: "x,y,z", Value: Set of pieces
  ...
  add(piece) {
    const key = this.getCellKey(piece.mesh.position);
    if (!this.cells.has(key)) {
      this.cells.set(key, new Set());
    }
    this.cells.get(key).add(piece);
  }

  getPotentialPieces(raycaster) {
    // Get intersection point with board plane
    // Create a horizontal plane at y = 0
    const plane = new THREE.Plane(new THREE.Vector3(0, 1, 0), 0);

    // Create a vector to store the intersection point
    const intersectPoint = new THREE.Vector3();

    // Find the intersection point of the ray with the plane
    raycaster.ray.intersectPlane(plane, intersectPoint);

    // Get the cell key based on the intersection point
    const key = this.getCellKey(intersectPoint);
...
export default class Scene extends Component {
  componentDidMount() {
    ...
    this.setState({
      ...
      numConsecutiveWins: window.localStorage.getItem("wins")
    ...
    var light2 = new THREE.SpotLight(
      spotLightProps2.color,
      spotLightProps2.intensity
    );
    light2.position.set(
      -spotLightProps2.position.x,
      spotLightProps2.position.y,
      spotLightProps2.position.z
    );
    ...
    Promise.all([
      ...
      board.scale.set(
        modelProps.board.scale,
        modelProps.board.scale,
        modelProps.board.scale
      );
      board.position.set(
        modelProps.board.position.x,
        modelProps.board.position.y,
        modelProps.board.position.z
      );
      scene.add(board);
      ...
      for (let i = 0; i < boardSize; i++) {
        ...
        for (let j = 0; j < boardSize; j++) {
          ...
          tileMesh.scale.set(
            modelProps.cell.scale,
            modelProps.cell.scale,
            modelProps.cell.scale
          );
          tileMesh.children[0].material = tileMesh.children[0].material.clone();
          tileMesh.material = tileMesh.children[0].material;
          tileMesh.material.color =
            (i + j) % 2
              ? new THREE.Color(lightTone)
              : new THREE.Color(darkTone);
          tileMesh.position.set(
            j * tileSize - tileSize * 3.5 + 0.035,
            0.5,
            -(i * tileSize - tileSize * 3.5)
          );
      ...
      if (this.props.mode === gameModes["P2P"]) {
        ...
        this.socket.on(
          socketEvents["SC_GameStarted"],
          this.handleGameStarted.bind(this)
        );
        this.socket.on(
          socketEvents["SC_ChangeTurn"],
          this.handleChangeTurn.bind(this)
        );
        this.socket.on(
          socketEvents["SC_PlayerLogOut"],
          this.handlePlayerLogOut.bind(this)
        );
        this.socket.on(
          socketEvents["SC_ForceExit"],
          this.handleForceExit.bind(this)
        );
        this.socket.on(
          socketEvents["SC_SelectPiece"],
          this.handleSelectPiece.bind(this)
        );
        this.socket.on(
          socketEvents["SC_PawnTransform"],
          this.handlePawnTransform.bind(this)
        );
        this.socket.on(
          socketEvents["SC_PerformMove"],
          this.handlePerformMove.bind(this)
        );
        this.socket.on(
          socketEvents["SC_UnSelectPiece"],
          this.handleUnSelectPiece.bind(this)
        );
        this.socket.on(
          socketEvents["SC_RemainingTime"],
          this.handleRemainingTime.bind(this)
        );
        this.socket.on(
          socketEvents["SC_ActivateItem"],
          this.handleActivateItem.bind(this)
        );
        this.socket.on(
          socketEvents["SC_ItemInfo"],
          this.handleItemInfo.bind(this)
        );
        this.socket.on(
          socketEvents["SC_SendDrawRequest"],
          this.handleSendDrawRequest.bind(this)
        );
        this.socket.on(
          socketEvents["SC_DrawMatch"],
          this.handleDrawMatch.bind(this)
        );
      } else {
        this.setState({
        ...
        if (
          this.props.mode === gameModes["P2E"] &&
          this.props.side === "black"
        ) {
        ...
        if (this.props.mode === gameModes["P2E"]) {
          this.setState({
            myTurn:
              this.props.side === this.props.game.board.configuration.turn,
          });

          const aiNames = ["AI MonKey", "Beginner", "Intermediate", "Advanced"];
    ...
    var mouseDownAction = function (event) {
      ...
      if (
        self.props.mode === gameModes["P2P"] &&
        self.currentPlayer !== self.socket.id
      ) {
        return;
      } else if (
        self.props.mode === gameModes["P2E"] &&
        self.props.game.board.configuration.turn !== self.props.side
      ) {
        return;
      }
      ...
      mouse.x =
        ((event.clientX -
          (window.innerWidth - renderer.domElement.clientWidth) / 2) /
      ...
      mouse.y =
        -(
          (event.clientY -
            (window.innerHeight - renderer.domElement.clientHeight) / 2) /
      ...
      if (
        ...
        self.state.currentItem !== heroItems["jumpyShoe"]
      ) {
        for (let i = 0; i < self.boardGroundArray.length; i++) {
          for (let j = 0; j < self.boardGroundArray.length; j++) {
            const intersect = raycaster.intersectObject(
              self.boardGroundArray[i][j].mesh
            );
            if (intersect.length > 0) {
              // select on the board cell
              const effectArray = [];
              ...
              if (self.state.currentItem === heroItems["iceWall"]) {
                for (let t = -1; t <= 1; t++) {
                  if (self.boardGroundArray[i][j + t]) {
                    effectArray.push(
                      getFenFromMatrixIndex(
                        self.boardGroundArray[i][j + t].rowIndex,
                        self.boardGroundArray[i][j + t].colIndex
                      )
                    );
                  }
                }
              ...
              if (self.state.currentItem === heroItems["petrify"]) {
                effectArray.push(
                  getFenFromMatrixIndex(
                    self.boardGroundArray[i][j].rowIndex,
                    self.boardGroundArray[i][j].colIndex
                  )
                );
              }
      ...
      const myPiecesArray = self.boardPiecesArray.filter((item) => {
        if (self.props.mode === gameModes["practise"]) {
          ...
          return (
            (self.props.side === "white" &&
              item.pieceType === item.pieceType.toUpperCase()) ||
            (self.props.side === "black" &&
              item.pieceType !== item.pieceType.toUpperCase())
          );
      ...
      const grid = new SpatialGrid(self.boardPiecesArray.length); // Cell size based on your board scale
      myPiecesArray.forEach((piece) => grid.add(piece));

      // Intersection check
      const potentialPieces = grid.getPotentialPieces(raycaster);
      // looping through the potential pieces instead of all pieces
      for (const piece of potentialPieces) {
        const intersect = raycaster.intersectObject(piece.mesh);
        if (intersect.length > 0) {
          const { rowIndex, colIndex } = piece;
          const fen = getFenFromMatrixIndex(rowIndex, colIndex);

          if (self.props.mode === gameModes["P2P"]) {
            self.socket.emit(socketEvents["CS_SelectPiece"], {
              fen,
            ...
            self.possibleMoves = self.props.game.moves(fen);
          }
          return;
        }
      }
      ...
      if (self.selectedPiece) {
        for (let i = 0; i < self.possibleMoves.length; i++) {
          ...
          const groundMesh =
            self.boardGroundArray[groundIndex.rowIndex][groundIndex.colIndex]
              .mesh;
          const intersect = raycaster.intersectObject(groundMesh);

          if (intersect.length > 0) {
            // selected the possible move points
            const from = getFenFromMatrixIndex(
              self.selectedPiece.rowIndex,
              self.selectedPiece.colIndex
            );
            const to = getFenFromMatrixIndex(
              groundIndex.rowIndex,
              groundIndex.colIndex
            );
            if (self.props.mode === gameModes["P2P"]) {
          ...
          for (let j = 0; j < self.boardGroundArray[i].length; j++) {
            ...
            if (intersect.length > 0) {
                ...
                for (let t = -1; t <= 1; t++) {
                  ...
                  if (!activeBoard) {
                    ...
                    const position = getMeshPosition(
                      activeBoard.rowIndex,
                      activeBoard.colIndex
                    );
                    const pieceIndex = self.boardPiecesArray.findIndex(
                      (item) =>
                        item.rowIndex === activeBoard.rowIndex &&
                        item.colIndex === activeBoard.colIndex
                    );

                    self.currentMouseMeshes[t + 1].children[0].material =
                      self.currentMouseMeshes[
                        t + 1
                      ].children[0].material.clone();
                    self.currentMouseMeshes[t + 1].material =
                      self.currentMouseMeshes[t + 1].children[0].material;

                    if (pieceIndex === -1) {
                      self.currentMouseMeshes[t + 1].material.color =
                        new THREE.Color("#50d760");
                    } else {
                      self.currentMouseMeshes[t + 1].material.color =
                        new THREE.Color("#d75050");
                    }

                    self.currentMouseMeshes[t + 1].position.set(
                      position.x + 0.1,
                      1,
                      position.z + 0.06 - 0.5
                    );
              ...
              if (self.state.currentItem === heroItems["petrify"]) {
                ...
                self.currentMouseMeshes[0].children[0].material =
                  self.currentMouseMeshes[0].children[0].material.clone();
                self.currentMouseMeshes[0].material =
                  self.currentMouseMeshes[0].children[0].material;

                if (pieceIndex === -1) {
                  self.currentMouseMeshes[0].material.color = new THREE.Color(
                    "#d75050"
                  );
                } else {
                  self.currentMouseMeshes[0].material.color = new THREE.Color(
                ...
                self.currentMouseMeshes[0].position.set(
                  position.x,
                  0.6,
                  position.z
                );
    ...
    var performMove = (moveResult) => {
      ...
      if (this.props.game.board.configuration.turn === "white") {
        if (
          this.boardPiecesArray[fromIndex].pieceType === "K" &&
          to === "C1" &&
          this.props.game.board.configuration.castling.whiteLong
        ) {
          ...
          this.boardPiecesArray[fromIndex].pieceType === "K" &&
          to === "G1" &&
          this.props.game.board.configuration.castling.whiteShort
        ) {
        ...
        if (
          this.boardPiecesArray[fromIndex].pieceType === "k" &&
          to === "C8" &&
          this.props.game.board.configuration.castling.blackLong
        ) {
          ...
          this.boardPiecesArray[fromIndex].pieceType === "k" &&
          to === "G8" &&
          this.props.game.board.configuration.castling.blackShort
        ) {
      ...
      if (
        (currentTurn === "white" &&
          ...
          this.boardPiecesArray[fromIndex].rowIndex === 7) ||
        (currentTurn === "black" &&
          this.boardPiecesArray[fromIndex].pieceType === "p" &&
          this.boardPiecesArray[fromIndex].rowIndex === 0)
      ) {
        if (
          this.props.mode === gameModes["P2E"] &&
          this.props.side !== currentTurn
        ) {
          // AI turn
          ...
          this.boardPiecesArray[fromIndex].mesh.rotation.y =
            type === type.toUpperCase() ? Math.PI : 0;
    ...
    var animate = function () {
      ...
      if (self.moveFinished()) {
        if (self.props.mode === gameModes["P2P"] && self.isFinished) {
          if (self.side !== self.currentTurn) {
            ...
            if (!self.props.friendMatch) {
              window.localStorage.setItem(
                "wins",
                parseInt(self.state.numConsecutiveWins) + 1
              );

              if (
                (window.localStorage.getItem("chance") == null) |
                (window.localStorage.getItem("chance") == "1")
              )
          ...
          if (self.timeInterval) clearInterval(self.timeInterval);
          return;
        }
      }
      ...
      if (self.selectedPiece) {
        ...
        const speed = 0.05;
        if (
          self.selectedPiece.mesh.position.y >
            self.selectedPiece.currentY + maxHeight ||
          self.selectedPiece.mesh.position.y < self.selectedPiece.currentY
        ) {
          self.selectedPiece.animateDirection =
            -self.selectedPiece.animateDirection;
        }
        self.selectedPiece.mesh.position.y +=
          self.selectedPiece.animateDirection * speed;
      }
      ...
      for (let i = 0; i < boardSize; i++) {
        for (let j = 0; j < boardSize; j++) {
          self.boardGroundArray[i][j].mesh.material.color.setStyle(
            self.boardGroundArray[i][j].color
          );
        }
      }
      ...
      if (self.props.mode === gameModes["P2P"]) {
        if (self.lastMoveHistory) {
          ...
          self.boardGroundArray[toMatrixIndex.rowIndex][
            toMatrixIndex.colIndex
          ].mesh.material.color.setStyle(historyTone);

          const fromMatrixIndex = getMatrixIndexFromFen(fromHistory);
          self.boardGroundArray[fromMatrixIndex.rowIndex][
            fromMatrixIndex.colIndex
          ].mesh.material.color.setStyle(historyTone);
        }
      } else {
      ...
      if (
        self.props.mode === gameModes["P2P"] &&
        self.dangerKing &&
        (self.dangerKing["K"] || self.dangerKing["k"])
      ) {
        const pieceType = self.dangerKing["K"] ? "K" : "k";
        const kIndex = self.boardPiecesArray.findIndex(
          (item) => item.pieceType === pieceType
        );
        if (kIndex !== -1) {
          const rowIndex = self.boardPiecesArray[kIndex].rowIndex;
          const colIndex = self.boardPiecesArray[kIndex].colIndex;

          self.boardGroundArray[rowIndex][
            colIndex
          ].mesh.material.color.setStyle(dangerTone);
        }
      } else {
        const pieceType =
          self.props.game.board.getPlayingColor() === "white" ? "K" : "k";
      ...
      if (self.selectedPiece && self.possibleMoves) {
        self.possibleMoves.forEach((pos) => {
          ...
          self.boardGroundArray[matrixIndex.rowIndex][
            matrixIndex.colIndex
          ].mesh.material.color.setStyle(selectTone);
        });
      }
      ...
      self.boardPiecesArray.forEach((item) => {
        if (
          item.moveAnim &&
          !isSamePoint(item.moveAnim.target, item.mesh.position)
        ) {
  ...
  makeDeposit = async (roomId) => {
    ...
    if (res.transactionHash) {
      ...
      let tx2 = await llgRewardContract.deposit(
        ethers.BigNumber.from(roomId),
        ethers.utils.getAddress(this.props.wallet),
  ...
  determineIfHasBonus = async () => {
    ...
    if (
      (numConsecutiveWins == "3") |
      (numConsecutiveWins == "5") |
      (numConsecutiveWins == "10")
    ) {
      this.setState({
        numConsecutiveWins,
  ...
  getBonusReward = async () => {
    try {
      ...
      let tx = await llgRewardContract.giveBonusReward(
        ethers.utils.getAddress(wallet),
        ethers.BigNumber.from(this.props.roomKey),
        ethers.BigNumber.from(123),
        ethers.BigNumber.from(this.state.numConsecutiveWins),
        {
          value: 0,
          from: this.props.wallet,
        }
  ...
  getWinningRewards = async () => {
    ...
    let tx2 = await llgRewardContract.offerWinningReward(
      ethers.BigNumber.from(this.props.roomKey),
      ethers.BigNumber.from(123),
      ethers.utils.getAddress(this.props.wallet),
      this.props.friendMatch,
      {
        value: 0,
        from: this.props.wallet,
      }
  ...
  getRefund = async () => {
    ...
    let tx2 = await llgRewardContract.refund(
      ethers.BigNumber.from(this.props.roomKey),
      ethers.BigNumber.from(123),
      ethers.utils.getAddress(this.props.wallet),
      ethers.BigNumber.from(refundAmount),
      {
        value: 0,
        from: this.props.wallet,
      }
  ...
  pawnTransform(type) {
    ...
    const targetPiece =
      this.boardPiecesArray[this.state.pawnTransProps.fromIndex];
    ...
    if (this.props.mode === gameModes["P2P"]) {
      ...
      this.props.game.move(
        this.state.pawnTransProps.from,
        this.state.pawnTransProps.to
      );
      ...
      if (this.props.mode === gameModes["P2E"]) {
        // ai action after select the piece
  ...
  selectPiece(piece) {
    if (this.selectedPiece) {
      if (this.selectedPiece.mesh.uuid === piece.mesh.uuid) {
        // TODO : do nothing when reselect the current selected chess piece
        return;
      } else {
  ...
  setObstacles(obstacleArray) {
    ...
    obstacleArray.forEach((obstacle) => {
      if (obstacle.type === heroItems["iceWall"]) {
        const mesh = this.meshArray["iceWall"].clone();
        const position = getMeshPosition(
          getMatrixIndexFromFen(obstacle.position)["rowIndex"],
          getMatrixIndexFromFen(obstacle.position)["colIndex"]
        );
  ...
  handlePawnTransform(params) {
    if (this.side !== this.currentTurn) return;
  ...
  render() {
    return (
      <div className="GameScene">
        ...
        {/* Claim modal */}
        <Claim
          show={this.state && this.state.showClaimModal}
          msg={`Congratulation, You won ${
            this.state && this.state.numConsecutiveWins
          } matches in a row. You earn ${
            this.state && this.state.bonusReward
          } LGG more!`}
          onClickClaim={this.onClickClaim}
          btnText={
            this.state && this.state.wallet ? "Claim Reward" : "Connect Wallet"
          }
        />

        {/* Victory modal */}
        <Victory
          show={this.state && this.state.showVictoryModal}
          tax={this.state && this.state.tax}
          roomName={this.props.roomName}
          onClickLLGSymbol={this.onClickLLGSymbol}
        />
        ...
        {this.state && this.state.showWaitingModal ? (
          <Loading
            title={this.state.waitingModalTitle}
        ...
        <Popup
          ...
          type={"drawRequest"}
          message={
            "The opponent player wants to draw the match. Are you agree with it?"
          }
